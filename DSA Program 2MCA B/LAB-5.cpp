...
void fractionalKnapsack() {
    ...
    printf("Enter profit and resource (weight) for each task:\n");
    for (i = 0; i < n; i++) {
    ...
    for (i = 0; i <= n; i++) {
        for (int j = 0; j <= w; j++) {
            if (i == 0 || j == 0)
                ...
                dp[i][j] = (profit[i - 1] + dp[i - 1][j - weight[i - 1]] > dp[i - 1][j]) ?
                            (profit[i - 1] + dp[i - 1][j - weight[i - 1]]) : dp[i - 1][j];
            else
                dp[i][j] = dp[i - 1][j];
        }
    }
...
void primMST() {
    ...
    scanf("%d", &n);
    int cost[MAX][MAX], selected[MAX], no_edge = 0;
    printf("Enter cost matrix (0 for no connection):\n");
    for (int i = 0; i < n; i++)
        for (int j = 0; j < n; j++) {
            scanf("%d", &cost[i][j]);
...
void kruskalMST() {
    int n, edges;
    printf("\nEnter number of servers and connections: ");
    scanf("%d %d", &n, &edges);
    int u[edges], v[edges], w[edges];
    printf("Enter connections (u v cost):\n");
